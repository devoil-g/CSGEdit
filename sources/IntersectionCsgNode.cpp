...
size_t				RT::IntersectionCsgNode::build(std::vector<RT::OpenCL::Node> & nodes, std::vector<RT::OpenCL::Primitive> & primitives, Math::Matrix<4, 4> const & transformation, RT::Material const & material, unsigned int deph) const
{
  if (children().size() == 1)
  ...
  nodes.back().type = RT::OpenCL::Node::Type::TypeIntersection;
